...
// 点击日期，返回所选日期，并隐藏面板
function backDate(tab,inp,btn,y,m,arr) {
    var atr=tab.tBodies[0].rows;
    var k,n,i,j;
    var flag=false;
	...
	okbtn.onclick=function () {
	    inp.value=null;
	    if (arr.length==2) {
	        arr.sort();// 日期排序    20160429
	        // 格式化字符串 2016-04-29
	        arr[0]=formatStr(arr[0]);
	        arr[1]=formatStr(arr[1]);
	        inp.value=arr[0]+"to"+arr[1];
	    }
	    else {
	        inp.value=formatStr(arr[0]);
	    }
	    arr.length=0; //归零
    	tab.style.visibility="hidden";
    };
...
// 格式化日期字符串 from '20160429'  to '2016-04-29'
