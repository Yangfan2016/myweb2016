...
// 主函数
function mainF() {
    var aInp=document.querySelectorAll("#f2 input");
    var aCbox=document.querySelectorAll("#f1 input[type='checkbox']");
    var oSubmit=document.querySelector("#btn");
    var oSpan=document.querySelectorAll("#f2 span");
    var aLabel=document.querySelectorAll("#f2 label");
    ...
    // 获取 工厂选项
    for (var k=0;k<aCbox.length;k++) {
        // 初始化
        aInp[k].setAttribute("class","hide");
        aLabel[k].setAttribute("class","hide");
        oSpan[k].setAttribute("class","hide");
        aLabel[k].lab=aLabel[k].innerHTML;
        aLabel[k].innerHTML="";
        oSpan[k].innerHTML="";

        aCbox[k].index=k;
        aCbox[k].onclick=function () {
            var that=this.index;
            if (!aCbox[that].checked) {
                aInp[that].setAttribute("class","hide");
                aLabel[that].setAttribute("class","hide");
                oSpan[that].setAttribute("class","hide");
                aLabel[that].innerHTML="";
            }
            else {
                aInp[that].className="";
                aLabel[that].className="";
                oSpan[that].className="";
                aLabel[that].innerHTML=aLabel[that].lab;
            }
